...
- Anki:
  ...
  - The bookshelf exposes an app-owned-book-only Sasayaki setup panel with status, missing external-source state, distinct link/copy import actions, scoped removal confirmation, and explicit matching/playback deferral. The bookshelf probe covers action wiring and external-file protection; wide and 520px checks passed without horizontal overflow.
