...
3. Treat Anki remote word-audio Slice 5B, HSA-compatible local-audio Slice 5C, and default-off post-add sync Slice 5D in `docs/ANKI_AUDIO_SYNC_PLAN.md` as complete at automated-validation level. Next recommended slice: tags and duplicate-request policy (Slice 5E). Do not claim real remote-audio, real HSA local-audio database, real sync, or combined add-note-plus-dictionary-media runtime verification until suitable fixtures are available.
