...
3. Treat Anki remote word-audio Slice 5B and HSA-compatible local-audio Slice 5C in `docs/ANKI_AUDIO_SYNC_PLAN.md` as complete at automated-validation level. Next recommended slice: default-off optional AnkiConnect sync (Slice 5D). Do not claim real remote-audio, real HSA local-audio database, or combined add-note-plus-dictionary-media runtime verification until suitable fixtures are available.
