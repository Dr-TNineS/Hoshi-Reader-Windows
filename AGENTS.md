...
- 长期需要对照的上游参考项目通过 Git submodule 固定在 `reference/` 下：`reference/Hoshi-Reader-Android` 是 HSA 主参考，`reference/hoshi-reader-mac` 是 HSM 补充参考。
...
- 除 HSA/HSM 外，不要向 `reference/` 添加新的长期参考项目，除非任务明确扩展参考范围并同步更新 `.gitmodules`、`reference/README.md` 与本规则。
