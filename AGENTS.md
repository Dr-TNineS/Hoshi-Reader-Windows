...
- 在 PowerShell 中不得用外层双引号直接包包含 `C:\Program Files (x86)\...` 的 `cmd /c "call ..."` 命令；这会让 `(x86)` 路径被 PowerShell 拆坏。优先使用 `scripts\vs-x64-cargo.cmd`；确需手写 `cmd /c` 时，用外层单引号包整条命令，例如 `cmd /c 'call "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\Common7\Tools\VsDevCmd.bat" -arch=x64 && cargo check'`。
